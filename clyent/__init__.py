...
def json_group(group):
    grp_data = {
        ...
        "title": group.title,
        "actions": [
            json_action(a) for a in group._group_actions if a.help != argparse.SUPPRESS
        ],
    }
...
def add_default_arguments(parser, version=None):
    ...
    ogroup.add_argument(
        ...
        "--quiet",
        action="store_const",
        help="Only show warnings or errors the console",
        dest="log_level",
    ...
    if version:
        parser.add_argument(
            ...
            "--version",
            action="version",
            version="%(prog)s Command line client (version {})".format(version),
        )
