...
COLOR_MAP = {logging.ERROR: "red", logging.WARN: "yello", logging.DEBUG: "blue"}


class ColorStreamHandler(logging.Handler):
    def __init__(
        self, level=logging.INFO, show_tb="tty", exceptions=(errors.ClyentError,)
    ):
    ...
    def emit(self, record):
        ...
        should_hide = (
            True
            if self.show_tb == "never"
            else False
            if self.show_tb == "always"
            else stream.isatty()
        )
...
def main():
    ...
    #     FormatterWrapper.wrap(
    #     h, prefix=color('prefix |', [color.WHITE, color.BACKGROUND_COLORS[0]])
    #     )
    try:
        asdf
    except NameError:
